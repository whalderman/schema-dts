...
import { Literal, NamedNode, Quad, type Store } from "n3";
	...
	describe("with one or more message", () => {
		...
		describe(".nt syntax errors", () => {
			...
			it("Non-IRI Subject", async () => {
				const control = fakeResponse(200, "Ok");
				control.data(
					`"knowsAbout" <https://scema.org/domainIncludes> "a" .`
				);
				control.end();
				await expect(store).rejects.toThrow();
			});
			...
			it("Non-IRI Predicate", async () => {
				const control = fakeResponse(200, "Ok");
				control.data(
					`<https://schema.org/knowsAbout> "domainIncludes" "a"`
				);
				control.end();
				await expect(store).rejects.toThrow();
			});
			...
			it("Invalid object", async () => {
				const control = fakeResponse(200, "Ok");
				control.data(
					`<https://schema.org/a> <https://schema.org/b> 'c`
				);
				control.end();
				await expect(store).rejects.toThrow();
			});
			...
			it("Domain only", async () => {
				const control = fakeResponse(200, "Ok");
				control.data(
					`<https://schema.org/> <https://schema.org/b> "c"`
				);
				control.end();
				await expect(store).rejects.toThrow();
			});
		...
		describe("local file", () => {
			...
			beforeEach(() => {
				const mockFileLine =
					"<https://schema.org/Person> <https://schema.org/knowsAbout> <https://schema.org/Event> .\n";
				readFileFn = jest
					.spyOn(fs, "readFile")
					.mockImplementation((path) =>
						Promise.resolve(mockFileLine)
					);
			});
			it("fails loading a file (containing .nt syntax errors)", async () => {
				const failingMockPath = "./bad-ontology.nt";
				const failingMockLine =
					"<https://schema.org/knowsAbout> <https://sc";
				readFileFn.mockImplementation((path) =>
					Promise.resolve(failingMockLine)
				);
			...
			it("loads a file from the correct path", async () => {
				...
				expect(
					(await fileStore).getQuads(null, null, null, null)
				).toEqual([
					new Quad(
						new NamedNode("https://schema.org/Person"),
						new NamedNode("https://schema.org/knowsAbout"),
