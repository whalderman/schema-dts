...
describe("Context.getScopedName", () => {
	it("with single domain URL (https)", () => {
		...
		expect(
			ctx.getScopedName(new NamedNode("https://schema.org/Thing"))
		).toBe("Thing");
		expect(
	...
	it("with single domain URL (http)", () => {
		...
		expect(
			ctx.getScopedName(new NamedNode("http://schema.org/Thing"))
		).toBe("Thing");
		expect(
			ctx.getScopedName(new NamedNode("http://schema.org/rangeIncludes"))
		).toBe("rangeIncludes");
		expect(
			ctx.getScopedName(new NamedNode("https://schema.org/Door"))
...
describe("Context.Parse", () => {
	...
	it("two named, extra commas", () => {
		expect(
			Context.Parse(
				",a:https://schema.org/A ,, ,\tb:https://schema.org/B,,"
			)
		).toEqual(
			ctx((c) => {
				c.addNamedContext("a", "https://schema.org/A");
