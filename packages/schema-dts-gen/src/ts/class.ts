...
import { type Property, TypeProperty } from "./property.js";
	...
	protected nonEnumType(skipDeprecated: boolean): TypeNode[] {
		...
		const children: TypeNode[] = this.children
			...
			.map((child) =>
				factory.createTypeReferenceNode(
					child.className(),
					/*typeArguments=*/ child.typeArguments(
						this.typeParameters()
					)
				)
			);
...
export function Sort(a: Class, b: Class): number {
	...
	if (a instanceof DataTypeUnion) {
		return b instanceof DataTypeUnion ? 0 : -1;
	}
	if (b instanceof DataTypeUnion) {
